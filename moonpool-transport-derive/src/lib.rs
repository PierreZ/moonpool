...
fn interface_impl(attr: InterfaceAttr, item: ItemTrait) -> syn::Result<proc_macro2::TokenStream> {
    ...
    let expanded = quote! {
        ...
        impl<C: moonpool_transport::MessageCodec + Clone> #server_name<C> {
            ...
            pub fn init<N, T, TP, R>(transport: &std::rc::Rc<moonpool_transport::NetTransport<N, T, TP, R>>, codec: C) -> Self
            where
                ...
                R: moonpool_transport::RandomProvider + Clone + 'static,
            {
                #(#server_inits)*
        ...
        impl #client_name {
            ...
            /// ```
            pub fn bind<N, T, TP, R, C>(
                &self,
                transport: std::rc::Rc<moonpool_transport::NetTransport<N, T, TP, R>>,
                codec: C,
            ) -> #bound_client_name<N, T, TP, R, C>
            where
                N: moonpool_transport::NetworkProvider + Clone + 'static,
                T: moonpool_transport::TimeProvider + Clone + 'static,
                TP: moonpool_transport::TaskProvider + Clone + 'static,
        ...
        pub struct #bound_client_name<N, T, TP, R, C>
        where
            N: moonpool_transport::NetworkProvider + Clone + 'static,
            T: moonpool_transport::TimeProvider + Clone + 'static,
            TP: moonpool_transport::TaskProvider + Clone + 'static,
        ...
        impl<N, T, TP, R, C> #bound_client_name<N, T, TP, R, C>
        where
            N: moonpool_transport::NetworkProvider + Clone + 'static,
            T: moonpool_transport::TimeProvider + Clone + 'static,
            TP: moonpool_transport::TaskProvider + Clone + 'static,
        ...
        #[async_trait::async_trait(?Send)]
        impl<N, T, TP, R, C> #name for #bound_client_name<N, T, TP, R, C>
        where
            N: moonpool_transport::NetworkProvider + Clone + 'static,
            T: moonpool_transport::TimeProvider + Clone + 'static,
            TP: moonpool_transport::TaskProvider + Clone + 'static,
