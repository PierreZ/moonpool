...
///
/// ```
/// use moonpool_foundation::NetworkAddress;
...
///
/// ```
/// use moonpool_foundation::{Endpoint, NetworkAddress, UID};
/// use std::net::{IpAddr, Ipv4Addr};
///
/// let addr = NetworkAddress::new(IpAddr::V4(Ipv4Addr::LOCALHOST), 4500);
/// let endpoint = Endpoint::well_known(addr, 1); // Ping endpoint
///
...
impl Endpoint {
    ...
    pub fn well_known(address: NetworkAddress, token_id: u32) -> Self {
        Self {
            address,
            token: UID::well_known(token_id),
        }
    }
