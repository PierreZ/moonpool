...
use moonpool::{FlowTransportBuilder, ReplyPromise, send_request};
use moonpool_foundation::{RandomProvider, sometimes_assert};
...
async fn multi_method_workload<N, T, TP>(
    random: moonpool_foundation::SimRandomProvider,
    network: N,
    time: T,
    task: TP,
    topology: moonpool_foundation::WorkloadTopology,
    num_operations: u64,
) -> moonpool_foundation::SimulationResult<moonpool_foundation::SimulationMetrics>
where
    ...
    TP: moonpool_foundation::TaskProvider + Clone + 'static,
{
    ...
    let local_addr = moonpool_foundation::NetworkAddress::new(
    ...
    let add_endpoint = moonpool_foundation::Endpoint::new(local_addr.clone(), add_token);
    let sub_endpoint = moonpool_foundation::Endpoint::new(local_addr.clone(), sub_token);
    let mul_endpoint = moonpool_foundation::Endpoint::new(local_addr.clone(), mul_token);
    ...
    Ok(moonpool_foundation::SimulationMetrics {
...
async fn build_listening_sim_workload<N, T, TP>(
    _random: moonpool_foundation::SimRandomProvider,
    network: N,
    time: T,
    task: TP,
    topology: moonpool_foundation::WorkloadTopology,
...
where
    ...
    TP: moonpool_foundation::TaskProvider + Clone + 'static,
{
    use moonpool::FlowTransportBuilder;

    // Create local address from topology
    let port = 4500u16;
    let ip: std::net::IpAddr = topology.my_ip.parse().expect("valid IP");
    let local_addr = moonpool_foundation::NetworkAddress::new(ip, port);
    ...
    Ok(moonpool_foundation::SimulationMetrics::default())
}
