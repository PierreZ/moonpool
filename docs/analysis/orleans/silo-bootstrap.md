...
## Reference Files
- `Silo.cs:1-677` - Main silo class, bootstrap orchestration, lifecycle participation
- `SystemTarget.cs:1-525` - Base class for infrastructure actors
...
- **Task.Run Execution**: All lifecycle operations run via `Task.Run` for thread pool isolation
...
4. **Self-participation**: Silo registers itself as a lifecycle participant (line 131)
...
// Silo's own participation - note each callback is wrapped in Task.Run()
...
**Task.Run wrapping**: Each lifecycle callback is wrapped in `Task.Run()` to execute on the thread pool, ensuring the lifecycle coordinator does not block on synchronous operations within callbacks.
...
**Critical Insight**: The lifecycle `OnStart` is dispatched via `Task.Run` to the thread pool. The lifecycle stages themselves execute **serially** within a single `OnStart` call -- each stage completes before the next begins.

**Why use Task.Run?**
1. **Thread pool isolation**: Lifecycle operations run on the thread pool, not the caller's context
2. **Ordered execution**: The `siloLifecycle.OnStart` method ensures sequential stage processing
3. **Error isolation**: Exceptions propagate cleanly back to the caller
4. **Simplicity**: No need for a dedicated SystemTarget just for lifecycle execution

**Note**: In earlier versions of Orleans, a `LifecycleSchedulingSystemTarget` was used to provide a `WorkItemGroup` for lifecycle execution. This has been removed in v10.0.0 in favor of the simpler `Task.Run` approach.

### Lifecycle Stage 1: RuntimeInitialize

**Code**: `Silo.cs:154-166, 369-381`
...
**CreateGrainServices** (`Silo.cs:227-234`):
```csharp
...
**RegisterGrainService** (`Silo.cs:236-254`):
```csharp
...
private async Task OnActiveStop(CancellationToken ct)
{
    ...
    if (this.messageCenter.Gateway != null)
    {
        ...
        {
            await Task.Run(
                () => this.messageCenter.Gateway.SendStopSendMessages(this.grainFactory, ct),
                CancellationToken.None).WaitAsync(ct);
        }
        catch (Exception exception)
        {
...
**StartGrainService** (`Silo.cs:256-271`):
```csharp
...
**Stop Phase** (shutdown):
1. **Send disconnect requests**: Notify clients silo is shutting down (via `Task.Run` for thread pool isolation, with `ct` for cancellation support)
...
public async Task StopAsync(CancellationToken cancellationToken)
{
    ...
    try
    {
        // Lifecycle stages stop in reverse order via Task.Run
        await Task.Run(
...
3. **Execute lifecycle.OnStop()**: Stages execute in **reverse order** via `Task.Run`
...
#### 3. Task.Run for Thread Pool Isolation

**Pattern**: All lifecycle operations dispatch through `Task.Run`

**Why?**
- **Thread pool execution**: Lifecycle callbacks run on thread pool threads
- **Isolation**: No blocking the caller's synchronization context
- **Error propagation**: Exceptions propagate cleanly via Task

**Note**: Earlier Orleans versions used a dedicated `LifecycleSchedulingSystemTarget` with a `WorkItemGroup` for lifecycle execution. In v10.0.0, this was simplified to `Task.Run`, since the lifecycle itself ensures sequential stage processing.
...
public abstract partial class SystemTarget : ISystemTarget, ISystemTargetBase, IGrainContext,
    IGrainExtensionBinder, ISpanFormattable, IDisposable, IGrainTimerRegistry,
...
4. **No Activation Lifecycle**
   - `Activate()` is a no-op (line 375)
   - `Deactivate()` is a no-op (line 378)
   - `Deactivated` returns `Task.CompletedTask` (line 381)

5. **No Migration**
   - `Rehydrate()` disposes context (lines 393-397)
   - `Migrate()` is a no-op (lines 399-402)
...
7. **Request Cancellation Support**
   - Implements `IGrainCallCancellationExtension` (new in v10.0.0)
   - Tracks running requests in `_runningRequests` dictionary
   - Supports `CancelRequestAsync` for cooperative request cancellation
...
**Note**: `MessagingTrace` is declared as `{ get; } = new(loggerFactory)` (auto-property with initializer), meaning it is created once and reused across all accesses. This avoids repeated allocations that would occur with an expression-bodied `=>` property.

### SystemTarget Message Processing

**Code**: `SystemTarget.cs:142-164, 345-369`
...
2. **Track request**: Add to `_runningRequests` dictionary (enables cancellation)
3. **Schedule on TaskScheduler**: Create `Task<Task>` with `_handleRequest` delegate, start on `WorkItemGroup.TaskScheduler`
4. **HandleNewRequest**: Invoke via `RuntimeClient`, remove from `_runningRequests` on completion
5. **Response handling**: `HandleResponse` routes responses to callback registry

**Key Insight**: SystemTargets use a **task-based scheduling model** with `WorkItemGroup.TaskScheduler`:
- `_handleRequest` is a cached `Func<object?, Task>` delegate (line 17) to avoid allocation per message
- `ExecutionContextSuppressor` prevents flowing the execution context into the task
- `_runningRequests` dictionary enables cooperative request cancellation (new in v10.0.0)
- `Task<Task>` pattern allows async execution within the TaskScheduler

**Difference from earlier versions**: The `RequestWorkItem` class has been removed. Instead, messages are scheduled directly as `Task<Task>` instances on the `WorkItemGroup.TaskScheduler`.

**Difference from regular grains**: No reentrancy logic, no activation lifecycle, no deactivation

### SystemTarget Timers

**Code**: `SystemTarget.cs:183-241, 404-419`
...
public IGrainTimer RegisterGrainTimer(Func<CancellationToken, Task> callback,
    TimeSpan dueTime, TimeSpan period)
{
    CheckRuntimeContext();
    ArgumentNullException.ThrowIfNull(callback);
    var timer = _shared.TimerRegistry.RegisterGrainTimer(
        this, (state, ct) => state(ct), callback,
        new() { DueTime = dueTime, Period = period, Interleave = true });
    return timer;
}
...
- **Multiple overloads**: Three `RegisterTimer`/`RegisterGrainTimer` overloads with varying state/cancellation support
- **CancellationToken support**: `RegisterGrainTimer` overloads pass a `CancellationToken` to the callback
...
4. **Task.Run for Lifecycle Isolation**
   - Orleans: Uses `Task.Run` to dispatch lifecycle operations to thread pool
   - Moonpool: Could use a dedicated spawned task or run inline
   ```rust
   ...
   // Option 2: Inline execution (sufficient for single-core)
   pub async fn start_lifecycle(&mut self) -> Result<()> {
       self.lifecycle.on_start().await
   }
   ```
...
Orleans dispatches lifecycle via `Task.Run` but the lifecycle itself ensures sequential stage processing. Each stage completes before the next begins.
