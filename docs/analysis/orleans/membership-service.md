...
## Reference Files
- `ClusterMembershipSnapshot.cs:1-133` - Immutable cluster membership snapshot
- `ClusterMember.cs:1-67` - Individual cluster member representation
- `ClusterMembershipService.cs:1-153` - Membership service facade
- `MembershipTableManager.cs:1-1183` - Storage orchestration and lifecycle
- `MembershipAgent.cs:1-519` - Local silo status management
- `SiloHealthMonitor.cs:1-472` - Per-silo failure detection
- `ClusterHealthMonitor.cs:1-460` - Cluster-wide monitoring orchestration
- `SiloStatusOracle.cs:1-143` - Read-only membership view
- `MembershipGossiper.cs:1-34` - Gossip protocol implementation
- `ISiloStatusOracle.cs:1-84` - Status oracle interface
...
**Code**: `ClusterMembershipSnapshot.cs:10-38` (attributes at line 10, constructor 18-22, Members at 31, Version at 38)
...
public ClusterMembershipUpdate CreateUpdate(ClusterMembershipSnapshot previous)
{
    ...
    {
        throw new ArgumentException($"Argument must have a previous version...");
    }
...
- Based on `SiloAddress`, `Status`, and `Name` (with `StringComparison.Ordinal` for Name)
- Enables snapshot diffing
- HashCode based only on `SiloAddress` via `GetConsistentHashCode()` (line 61)
...
**Code**: `MembershipTableManager.cs:828-854`
...
1. **Periodic refresh** (`MembershipTableManager.cs:227-267`):
...
2. **Suspect/kill processing** (`MembershipTableManager.cs:613-651`):
...
**Code**: `MembershipTableManager.cs:370-413`
...
private async Task<bool> TryUpdateMyStatusGlobalOnce(SiloStatus newStatus)
{
    ...
    if (newStatus == SiloStatus.Dead)
        myEntry.AddSuspector(myAddress, now);  // Add killer (myself) to suspect list for diagnostics
    ...
    if (ok)
    {
        ...
        // Build updated table locally and publish snapshot
...
- **Retry loop** (`MembershipTableManager.cs:293-323`) via `MembershipExecuteWithRetries` with exponential backoff
- **Unlimited retries** for contention (constant `NUM_CONDITIONAL_WRITE_CONTENTION_ATTEMPTS = -1`, line 20)

### Voting-Based Failure Detection

The voting system uses a **channel-based architecture**: public methods `TryToSuspectOrKill` (line 695) and `TryKill` (line 607) write requests to a bounded channel, which are processed asynchronously by `ProcessSuspectOrKillLists` (line 613). This separates the caller from the potentially slow storage operations.

There are two request types: `SuspectOrKill` (vote + potentially declare dead) and `Kill` (directly declare dead, used for cleanup of stale entries).

**Code**: `MembershipTableManager.cs:701-794` (InnerTryToSuspectOrKill)
    ...
    CancellationToken cancellationToken)
{
    ...
    // Bug detection: if enough votes already exist but silo not marked dead, something is wrong
    if (freshVotes.Count >= this.clusterMembershipOptions.NumVotesForDeathDeclaration)
    {
        // This is a bug - silo should already be dead
    ...
    {
        // Enough votes collected, declare dead
...
**DeclareDead** (`MembershipTableManager.cs:796-824`):
```csharp
private async Task<bool> DeclareDead(MembershipEntry entry, string etag, TableVersion tableVersion, DateTime time)
{
    if (this.clusterMembershipOptions.LivenessEnabled)
    {
        ...
        // Failed due to write conflicts, will retry
        return false;
    }

    // Liveness disabled, treat as success
    return true;
}
```
...
**Code**: `MembershipTableManager.cs:189-198`
...
**Code**: `MembershipTableManager.cs:542-571`
...
**RefreshFromSnapshot** (`MembershipTableManager.cs:108-132`):
```csharp
...
**Code**: `MembershipAgent.cs:289-367`
...
4. **OnRuntimeInitializeStop → Dead**
   - Disposes IAmAlive timer, cancels background tasks
   - Final status update (with 1 minute timeout)
   - Other silos will see this silo as dead

### ValidateInitialConnectivity

**Code**: `MembershipAgent.cs:117-223`
...
**Code**: `SiloHealthMonitor.cs:256-304`
...
**Code**: `SiloHealthMonitor.cs:313-366`
    ...
    CancellationToken cancellation)
{
    ...
    try
    {
        ...
        var indirectResult = await _prober.ProbeIndirectly(
            ...
            directProbeTimeout,
            id,
            cancellationSource.Token).WaitAsync(cancellationSource.Token);
...
**Code**: `ClusterHealthMonitor.cs:153-272`
    ...
    for (var ringNum = 0; ringNum < numProbedSilos; ++ringNum)
    {
        ...
        if (myIndex < 0)
        {
            throw new OrleansMissingMembershipEntryException(
                $"This silo {self.SiloAddress} status {self.Status} is not in its own local silo list!");
        }
...
**Code**: `ClusterHealthMonitor.cs:306-325`
...
**Code**: `ClusterHealthMonitor.cs:118-151`
...
private async Task EvictStaleStateSilos(MembershipTableSnapshot membership, DateTime utcNow)
{
    foreach (var member in membership.Entries)
    {
                ...
                maxJoinTime: this.clusterMembershipOptions.CurrentValue.MaxJoinAttemptTime))
        {
            ...
            {
                // Logs error with silo address and status
                log.LogError(exception, "...", member.Value.SiloAddress, member.Value.Status);
            }
        }
    }
...
**Code**: `SiloStatusOracle.cs:15-99`
...
**Code**: `SiloStatusOracle.cs:36-49, 101-129`
...
public SiloStatus GetApproximateSiloStatus(SiloAddress silo)
{
    ...
    if (status == SiloStatus.None)
    {
        ...
        {
            // Log that the silo is not registered
        }
    }
