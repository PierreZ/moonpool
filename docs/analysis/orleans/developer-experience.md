...
.AddCosmosGrainStorage("inventory", ...)     // Cosmos DB
```
...
Note: `RegisterGrainTimer` is an extension method on `IGrainBase` and returns an `IGrainTimer` (which implements `IDisposable`). Multiple overloads exist, including ones accepting `GrainTimerCreationOptions` for fine-grained control over interleaving and keep-alive behavior.
...
Note: `RegisterOrUpdateReminder` is an extension method on `Grain`/`IGrainBase` (requires `this.` prefix). The grain must implement the `IRemindable` interface to use reminders. `ReceiveReminder` is defined on `IRemindable` and is called by the runtime when a reminder fires.
...
```csharp
.AddCosmosGrainStorage("inventory", options =>
{
    options.ConfigureCosmosClient(accountEndpoint, authKeyOrResourceToken);
