...
**File**: `IPersistentState.cs:10-13`
...
public interface IStorage
{
    ...
    // With cancellation token overloads
...
- **Etag**: Optimistic concurrency token from storage
...
{
    public PersistentState(string stateName, IGrainContext context, IGrainStorage storageProvider)
        : base(stateName, context, storageProvider)
    {
        var lifecycle = context.ObservableLifecycle;

        // Subscribe to SetupState stage (before grain OnActivateAsync)
        lifecycle.Subscribe(RuntimeTypeNameFormatter.Format(GetType()), GrainLifecycleStage.SetupState, this);

        // Register as migration participant
...
**File**: `StateStorageBridge.cs:22-201`
...
public partial class StateStorageBridge<TState> : IStorage<TState>, IGrainMigrationParticipant
{
    ...
    public string? Etag
    {
        get => _grainState?.ETag;
        set => GrainState.ETag = value;
    }
    ...
    private void OnError(Exception exception, ErrorCode id, string operation)
    {
        ...
        _shared.Logger.LogError((int)id, exception,
            ...
            providerName, _shared.Name, operation, grainId, errorString);

        // Wrap non-Orleans exceptions
        if (exception is not OrleansException)
        {
            var errMsg = $"Error from storage provider {providerName}.{_shared.Name} during {operation} for grain {grainId}{errorString}\n {LogFormatter.PrintException(exception)}";
...
- **Instrumentation**: Timing and error metrics
...
**File**: `StateStorageBridge.cs:203-230`
...
**Pattern**: **Flyweight** - Share logger, activator, and store instance across multiple grains
...
**InsideRuntimeClient.cs:304-313**:
...
if (response.Exception is { } invocationException)
{
    ...
    {
        // Mark the exception so it doesn't deactivate any other activations
...
private void OnError(Exception exception, ErrorCode id, string operation)
{
    ...
    _shared.Logger.LogError((int)id, exception,
        "Error from storage provider {ProviderName}.{StateName} during {Operation} for grain {GrainId}",
        providerName, _shared.Name, operation, grainId);

    // Wrap non-Orleans exceptions
    if (exception is not OrleansException)
    {
        throw new OrleansException($"Error from storage provider...", exception);
    }
...
**Core Interfaces**:
- `IPersistentState.cs:10-13` - Grain-facing interface
- `IStorage.cs:99-106` - Storage interface with State, Etag, RecordExists
...
- `StateStorageBridge.cs:22-201` - Storage bridge with error handling and migration
- `StateStorageBridge.cs:203-230` - StateStorageBridgeShared (Flyweight)

**Error Handling**:
- `IGrainStorage.cs:59-177` - InconsistentStateException definition
- `InsideRuntimeClient.cs:304-313` - Automatic deactivation on InconsistentStateException
...
7. **Instrumentation** - Timing and error metrics

**Key Design Patterns**:
- **Flyweight** - Share logger, activator, storage provider
