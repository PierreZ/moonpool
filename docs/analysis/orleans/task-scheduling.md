...
## Reference Files
- `WorkItemGroup.cs:1-305` - Work group state machine and execution loop
- `ActivationTaskScheduler.cs:1-146` - Custom TaskScheduler for single-threaded execution
...
1. **Waiting → Runnable** (lines 98-115)
   ...
   lock (_lockObj)
   {
       ...
       ScheduleExecution(this);  // ThreadPool.UnsafeQueueUserWorkItem(this)
   }
   ```
...
3. **Running → Runnable** (lines 209-218)
...
4. **Running → Waiting** (lines 209-218)
...
3. **Per-task processing** (lines 157-194)
...
**Called from**: `ActivationData.AnalyzeWorkload()` (ActivationData.cs:773)
...
**Code**: `ActivationTaskScheduler.cs:66-72`
...
**Code**: `ActivationTaskScheduler.cs:82-136`
...
protected override bool TryExecuteTaskInline(Task task, bool taskWasPreviouslyQueued)
{
    ...
    {
        logger.LogTrace(
            "{TaskScheduler} TryExecuteTaskInline Task Id={TaskId} Status={Status} " +
            "PreviouslyQueued={PreviouslyQueued} CanExecute={CanExecute} Queued={Queued}",
...
**Code**: `SchedulingOptions` (referenced throughout)

Key settings:

1. **ActivationSchedulingQuantum** (line 147)
   - Default: typically 100ms (not visible in these files)
   - Controls how long activation can execute before yielding
   - 0 or negative: drain entire queue

2. **TurnWarningLengthThreshold** (line 146)
   - Default: typically 200ms
   - Threshold for logging long-running turns
   - Helps detect blocking code

3. **MaxPendingWorkItemsSoftLimit** (line 86)
   - Default: typically 0 (disabled)
