...
| `RpcError::is_never_retried`, `BalancedClient::with_credentials`, streaming methods refused by `BalancedClient::new`, `ServerShuttingDown` excluded like overload (final review) | `RetryPolicy` and the balancer kept separate, outdated contract-error lists: streaming mismatches and security refusals were retried or failed over. One list now; security refusals end a balanced call (policy is uniform across alternatives) and are never retried. Balanced calls could not carry per-client credentials. Real-TCP tests `credentials_reach_every_balanced_attempt` (+ flavors), `a_draining_server_is_failed_over_and_excluded`. |
...
- `bounded_campaign_hits_every_required_scenario`: seeds 1–30, each twice under `check_determinism`, 51 required scenarios, no violation, one record per run, every run recovered and at baseline; about 55–65 s in the debug profile on 4 cores (nextest slow-timeout override for this binary rather than a smaller budget). The same test over seeds 31–60 and 61–90 also fired every required scenario with no violation (90 seeds, 180 runs). The rarest required scenarios fire 6–8 times over the 60 runs of seeds 1–30 (so on at most 3–4 seeds).
- `cargo xtask sim run rpc-qualification` (sancov-instrumented, `until_coverage_stable(10, 1000)`, every seed twice under the canary), on the final commit: 122 seeds, 0 failed, 82 of 82 sometimes assertions hit, coverage stable at 28,407 of 70,760 edges for 10 seeds.
- The other campaigns re-run with `cargo xtask sim run` after the bit-flip mask (coverage-guided, every seed twice under the canary), all clean: `rpc-foundations` 69 seeds, 23/23 sometimes, 10,806 edges; `rpc-delivery` 36 seeds, 47/47, 11,530 edges; `rpc-interfaces` 71 seeds, 30/30, 11,908 edges; `rpc-streams` 134 seeds, 37/37, 13,015 edges; after the final review fixes (balancer failover classification, credentials): `rpc-balance` 62 seeds, 30/30, 11,346 edges; `rpc-security` 83 seeds, 47/47, 15,346 edges.
...
- Corruption is not combined with reboots, streams or balancing: bit flips stay the foundations campaign's explicit experiment and every other campaign masks them.
- The mutation checks ran on the campaign before the final review round (the oracles have only been tightened since).
