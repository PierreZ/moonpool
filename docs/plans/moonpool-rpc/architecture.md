...
| **Failure/reconnect policy.** `PeerPolicy` (in `RpcConfig::peer`): plain data for reconnect backoff (FDB `connectionKeeper`), ping interval/timeout, idle and inbound-idle timeouts, failure-detection delay, failure-cache bounds and inbound session sharing; the sim campaign buggifies every field. The `FailureMonitor` is concrete (address state, disconnect counts, bounded permanent endpoint failures, register-before-check waits). The call layer exposes per-attempt completion as `ReplyAttempt` (a future that still yields a late outcome when kept; `transmitted`, `cancel`) for #217. | #214 (done) |
| **Peers.** One selected connection per canonical remote address. `Hello` (protocol version 2) announces the sender's listen address, so an accepted session becomes the selected connection to its dialer; simultaneous dials settle as in FDB's `Peer::onIncomingConnection` (larger canonical address keeps its own dial); calls bound to a connection fail or, if reliable, are re-queued when it ends. | #214 (done) |
