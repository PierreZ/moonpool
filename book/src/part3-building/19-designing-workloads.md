...
**Conservation laws** check quantities that must remain constant. Total record count across shards. Messages sent minus messages received. Committed values in a consensus protocol must agree across replicas. The [events and invariants chapter](./17-events-and-invariants.md) covers this pattern in depth.
