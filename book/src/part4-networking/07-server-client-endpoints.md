...
The client side is even simpler. Create a `Client` with the server's address, then `bind()` it to a transport and codec:

```rust
let client = CalculatorClient::new(server_address);
let bound = client.bind(transport.clone(), JsonCodec);

let resp = bound.add(AddRequest { a: 1, b: 2 }).await?;
assert_eq!(resp.result, 3);
```

The bound client implements the service trait, so calling `bound.add(req)` is a real async function call that serializes the request, sends it over the wire, and awaits the response. Under the hood, `send_request` creates a temporary `ReplyFuture` registered at a unique endpoint, then the response arrives as a packet routed to that endpoint.
...
1. **Client** calls `bound.add(req)`, which calls `send_request`
