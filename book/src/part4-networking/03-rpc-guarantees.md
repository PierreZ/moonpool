...
**A reference names one incarnation, forever.** A process restarted at the same `ip:port` refuses its predecessor's references with `StaleIncarnation`, promptly and before any handler runs, whatever carries the reference: a direct call, a reliable call retained across the restart, a reply stream, a balanced set, a stored blob or a callback forwarded through a third process. Learning the new incarnation is an explicit application step (a directory lookup, a republication), never a silent refresh.
...
- **No confidentiality or peer authentication in plaintext mode.** `SecurityConfig::trusted_network()` claims nothing; credentials go over plaintext only when both sides opt in.
...
- **No platform beyond Moonpool's own.** Linux and macOS on Tokio (current-thread and multi-thread); on `wasm32-unknown-unknown`, the protocol and the simulation only, without the native TLS/JWT adapters.
...
The oracles are application ledgers only: boots, publications and executions written by handlers the moment user code runs; the streams campaign's producer and consumer ledgers; the security campaign's issue/receipt ledger; the balancer's observation hook. A seed's record (its history, the ledger digest and the campaign's trace events captured from the simulation timeline) must be identical when the seed runs again, after other seeds ran in the same process, and when every poll is slowed on the host by spinning or sleeping: no decision may come from wall-clock time.
