...
**Deactivation** -- The lifecycle event when an actor is removed from memory. The `on_deactivate` hook runs before the actor is dropped. Actors with `DeactivateOnIdle` hint deactivate after each message dispatch; `KeepAlive` actors stay resident until shutdown.
...
**Placement** -- The mechanism that determines which node hosts a virtual **actor**. In Moonpool, actors run on the node that receives their first message. Custom placement strategies can route actors to specific nodes based on their **identity** or tags.
