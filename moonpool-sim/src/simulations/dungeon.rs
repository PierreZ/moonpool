...
pub enum StepResult {
    /// Keep running.
    Continue,
    /// Bug found (treasure reached).
    BugFound,
    /// Reached step/death limit.
    Done,
}

/// Dungeon game wrapper with assertion instrumentation.
pub struct Dungeon {
    step_count: u64,
    max_steps: u64,
...
pub struct DungeonWorkload {
    /// Maximum steps per run.
    pub max_steps: u64,
    /// Target level (floor to reach for bug).
    pub target_level: u32,
}

impl Default for DungeonWorkload {
    fn default() -> Self {
        DungeonWorkload {
