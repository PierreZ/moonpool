...
impl NetworkProvider for SimNetworkProvider {
    ...
    fn bind(
        ...
        async move {
            ...
            let delay = sim.with_network_config(|config| {
                crate::network::sample_duration(&config.bind_latency)
            });
            ...
            let listener = SimTcpListener::new(sim_weak, addr);
            Ok(listener)
        }
    }
    ...
    fn connect(
        ...
        async move {
            ...
            let delay = sim.with_network_config(|config| {
                crate::network::sample_duration(&config.connect_latency)
            });

            // Create a connection pair for bidirectional communication
            let (client_id, server_id) = sim
                .create_connection_pair("client-addr".to_string(), addr.clone())
                .map_err(|e| {
                    io::Error::other(format!("Failed to create connection pair: {}", e))
                })?;

            // Store the server side for accept() to pick up later
            sim.store_pending_connection(&addr, server_id)
                .map_err(|e| {
                    io::Error::other(format!("Failed to store pending connection: {}", e))
                })?;
            ...
            let stream = SimTcpStream::new(sim_weak, client_id);
            Ok(stream)
        }
    }
}
