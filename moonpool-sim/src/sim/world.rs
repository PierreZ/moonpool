...
use super::{
    ...
    sleep::SleepFuture,
    state::{CloseReason, ConnectionState, ListenerState, NetworkState, StorageState},
...
impl SimWorld {
    ...
    pub fn step(&mut self) -> bool {
        ...
        if let Some(scheduled_event) = inner.event_queue.pop_earliest() {
            ...
            super::chaos_clog::clear_expired_clogs_inner(&mut inner);

            // Trigger random partitions based on configuration
            super::chaos_partition::randomly_trigger_partitions(&mut inner);
    ...
    pub(super) fn wake_all(
        wakers: &mut BTreeMap<ConnectionId, Vec<Waker>>,
        connection_id: ConnectionId,
    ) {
    ...
    fn handle_connection_event(inner: &mut SimInner, id: u64, state: ConnectionStateChange) {
        ...
        match state {
            ...
            ConnectionStateChange::PartitionRestore => {
                super::chaos_partition::clear_expired_partitions(inner);
            }
            ConnectionStateChange::SendPartitionClear => {
                super::chaos_partition::clear_expired_send_partitions(inner);
            }
            ConnectionStateChange::RecvPartitionClear => {
                super::chaos_partition::clear_expired_recv_partitions(inner);
            }
