...
/// `name` inside the scratch directory `dir`, as the `&str` path the provider
/// takes.
fn scratch_file(dir: &TempDir, name: &str) -> String {
    dir.path()
        .join(name)
        .to_str()
...
/// The bootstrap the provider refuses to do on the caller's behalf: create
/// `path` with an ordinary open, then make its bytes and its name durable.
async fn bootstrap(provider: &TokioStorageProvider, dir: &TempDir, path: &str) {
...
#[tokio::test]
async fn optional_direct_io_does_not_weaken_create_new() {
    let dir = TempDir::new().expect("temp dir");
    let path = &scratch_file(&dir, "exclusive.db");
...
/// one.
#[tokio::test]
async fn optional_direct_io_preserves_ordinary_open_failures() {
    let dir = TempDir::new().expect("temp dir");
    let missing = &scratch_file(&dir, "nonexistent.db");
    let provider = TokioStorageProvider::new();
...
#[tokio::test]
async fn optional_direct_io_applies_truncation_once() {
    let dir = TempDir::new().expect("temp dir");
    let path = &scratch_file(&dir, "truncated.db");
    let provider = TokioStorageProvider::new();
...
#[tokio::test]
async fn direct_io_constraints_describe_what_the_device_accepts() {
    let dir = TempDir::new().expect("temp dir");
    let path = &scratch_file(&dir, "aligned.db");
    let provider = TokioStorageProvider::new();
...
#[tokio::test]
async fn a_direct_io_file_refuses_stream_io() {
    use futures::io::{AsyncReadExt, AsyncSeekExt, AsyncWriteExt};

    let dir = TempDir::new().expect("temp dir");
    let path = &scratch_file(&dir, "stream.db");
    let provider = TokioStorageProvider::new();
...
#[tokio::test]
async fn a_buffered_file_keeps_stream_io() {
    use futures::io::{AsyncReadExt, AsyncSeekExt, AsyncWriteExt};

    let dir = TempDir::new().expect("temp dir");
    let path = &scratch_file(&dir, "buffered.db");
    let provider = TokioStorageProvider::new();
...
#[tokio::test]
async fn positioned_writes_ignore_append_on_a_real_file() {
...
#[tokio::test]
async fn a_required_open_serves_an_existing_file() {
    let dir = TempDir::new().expect("temp dir");
    let provider = TokioStorageProvider::new();
    let path = &scratch_file(&dir, "lifecycle.db");

    bootstrap(&provider, &dir, path).await;
...
#[tokio::test]
async fn a_required_open_never_creates_a_file() {
    let dir = TempDir::new().expect("temp dir");
    let provider = TokioStorageProvider::new();
    ...
    let target = &scratch_file(&dir, "bootstrap.db");
    bootstrap(&provider, &dir, target).await;
    ...
    let missing = &scratch_file(&dir, "absent.db");
    let error = provider
        .open(
            missing,
    ...
    for options in [
        ...
        let target = &scratch_file(&dir, "uncreated.db");
        let error = provider
            .open(target, options)
            .await
