...
| accounting | `crates/moonpool-assertions/src/{slots,buckets,region,..}.rs` | pure std, zero deps, wasm-able; `MAX_ASSERTION_SLOTS = 2048`, `SLOT_MSG_LEN = 64`, `MAX_EACH_BUCKETS = 256`, `MAX_EACH_KEYS = 6`; heap table by default, `install_region` overlays `MAP_SHARED` memory for the explorer |
